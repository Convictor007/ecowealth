...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { handlePost } = await import('../../server/appointments/handlePost')
    return await handlePost(req, res)
  } catch (err) {
    console.error('Appointment POST bootstrap error:', err)
    const errMsg = err instanceof Error ? err.message : String(err)
    return res.status(500).json({
      success: false,
      message: 'Booking API failed to load. Redeploy or check Vercel function logs.',
      detail: errMsg,
    })
  }
}
